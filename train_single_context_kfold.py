...
import sys
IGNORED_METRIC = str(sys.argv[1]) if len(sys.argv) > 1 else ''
...
def export_report(layer_name: str):
    ...
    with open(f"{fpath}/results.csv", 'a') as file:
        for model_name, result in repeated_result.items():
            file.write(f"{layer_name}, {model_name}-w/o-{IGNORED_METRIC}, {result.get_all_metrics_as_str()}\n")
        ...
        for layer_name in layer_names:
            ...
            try:
                ...
                Z = {1: ['size.1',
                    ...
                    'magnitude.3',
                    'scarcity.3'],
                    3: ['sin_t.1', 'cos_t.1', 'sin_t.2', 'cos_t.2', 'sin_t.3', 'cos_t.3'],
                    4: ['sd.1', 'magnitude.1', 'scarcity.1', 'sd.3', 'magnitude.3', 'scarcity.3'],
                    5: ['sd.1', 'magnitude.1', 'scarcity.1', 'sd.2', 'magnitude.2', 'scarcity.2'],
                    6: ['cos_t.1', 'sin_t.2', 'cos_t.2', 'sin_t.3', 'cos_t.3'],
                    7: ['diversity.1', 'diversity.2', 'diversity.3'],
                    8: ['size.1',
                    'popularity.1',
                    'diversity.1',
                    'size.3',
                    'popularity.3',
                    'diversity.3'],
                    9: ['diversity.1', 'size.2', 'popularity.2', 'diversity.2', 'diversity.3'],
                    10: ['temp_center_distance.1',
                    'temp_center_distance.2',
                ...
                for idx, (training, testing) in enumerate(get_k_folds(df, k=10)):
                    ...
                    try:
                        train_X, train_Y = sampler.sample_median_size(train_X, train_Y, max_size=NR_SAMPLES_K*1000)
